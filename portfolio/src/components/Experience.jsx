      ...
      <div className="section-container">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            {experiences.map((exp, index) => (
              <div key={index} className="relative pl-8 pb-10 last:pb-0 group">
                ...
                {index < experiences.length - 1 && (
                  <div className="absolute left-[11px] top-6 w-px h-full bg-inset" />
                )}
                {/* Timeline Dot */}
                <div
                ...
                <div className="glass-card glow-border p-6 hover:bg-inset/60 transition-all">
                  <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                    ...
                    <div className="text-right">
                      <span className="text-xs text-muted font-mono bg-inset px-2 py-1 rounded">
                        {exp.period}
                      </span>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {exp.tags.map((tag) => (
                      <span
                        key={tag}
                        className="text-xs px-2 py-0.5 rounded-full bg-inset text-body border border-line"
                      >
                        {tag}
                      </span>
                    ))}
          ...
          <div className="space-y-6">
            ...
            <div className="glass-card glow-border p-6">
              ...
              {education.map((edu, i) => (
                <div key={i} className="space-y-1">
                  <h4 className="text-sm font-medium text-heading">{edu.degree}</h4>
                  <p className="text-sm text-primary-400">{edu.institution}</p>
                  <p className="text-xs text-muted font-mono">{edu.period}</p>
                  <p className="text-xs text-muted mt-2">{edu.details}</p>
                </div>
              ))}
            </div>
            ...
            <a
              href="#"
              className="flex items-center justify-center gap-2 w-full px-6 py-3 bg-primary-600 hover:bg-primary-500 text-heading font-medium rounded-xl transition-all hover:shadow-lg hover:shadow-primary-500/25"
            >
