                    ...
                    {cat.skills.map((skill) => (
                      <span
                        ...
                        className={`inline-flex items-center text-sm px-4 py-2.5 rounded-xl border transition-all duration-200 hover:scale-105 cursor-default ${
                          cat.accent === 'brand'
                            ? 'bg-brand-500/5 text-accent-soft border-brand-500/15 hover:bg-brand-500/10 hover:border-brand-500/30'
                            : 'bg-warm-500/5 text-accent-warm border-warm-500/15 hover:bg-warm-500/10 hover:border-warm-500/30'
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8">
              ...
              <div className="space-y-2">
                {certifications.map((cert) => (
                  <div key={cert} className="flex items-center gap-2 text-sm text-body">
                ...
                <div className="glass p-6 hover:bg-inset/40 transition-all">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {exp.tags.map((tag) => (
                      <span
                        key={tag}
                        className="text-xs px-2 py-0.5 rounded-full bg-inset/80 text-body border border-line/50"
                      >
                        {tag}
                      </span>
                    ))}
            ...
            <div>
              ...
              {education.map((edu, i) => (
                <div key={i} className="glass p-6">
                  <h3 className="text-heading font-semibold mb-1">{edu.degree}</h3>
                  <p className="text-accent text-sm font-medium mb-1">{edu.institution}</p>
                  <p className="text-xs text-muted font-mono mb-3">{edu.period}</p>
